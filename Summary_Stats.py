...
# ----------------------------------------------------------------------------------#
home_dir = '/home/carterrhea/Dropbox/APLS-CAM-Proposal/DataFinal/Clean'
output_dir = '/home/carterrhea/Dropbox/APLS-CAM-Proposal/DataFinal/Output'
# ----------------------------------------------------------------------------------#


def summary_stats(home_dir, output_dir):
    """
    Main function to create histogram plots, summary statistics, and calculate network measures for each CAM.

    :param home_dir: Full path to raw data   e.g. '/home/user/Data/Clean'
    :param output_dir: Full path to clean data  e.g. 'home/user/Data/Output'
    :return: None
    """
    ...
    for filename in os.listdir(os.getcwd()):  # Step through files
        if filename.endswith('.csv'):
            ...
            if filename.endswith('_blocks.csv'):
                ...
                if num_blocks > 1:  # Only keep populated CAMS
    ...
    for name,cam_name in zip(names,cam_names):
        ...
        density, diameter, triadic_closure, central_node, central_node_title, central_node_val, central_node_valence, central_node_val_eig, central_node_val_bet = calc_density(df_blocks, df_links)
        network_dict[name] = [density, diameter, triadic_closure, central_node, central_node_title, central_node_val, central_node_valence]

    df_network = pd.DataFrame.from_dict(network_dict, orient='index',
                                      columns=['Density', 'Diameter', 'Triadic Closure', 'Central Node', 'Central Node Title', 'Central Node Value', 'Central Node Valence'])
    print(df_network)
...
#summary_stats(home_dir, output_dir)